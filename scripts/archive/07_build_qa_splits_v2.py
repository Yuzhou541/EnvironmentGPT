...
Q_PH = [
    "For dark fermentation biohydrogen production (e.g., from food waste), what pH is reported as optimal?",
    "In dark fermentation for biohydrogen, what is the optimal pH (or pH range) reported in the literature?"
]
Q_HRT = [
    "For dark fermentation biohydrogen production (e.g., from food waste), what HRT is reported as optimal?",
    "In dark fermentation for biohydrogen, what is the optimal hydraulic retention time (HRT) reported in the literature?"
]
...
def make_qa(f):
    ...
    cite = {"pdf_file": pdf, "chunk_id": chunk, "doi": f.get("doi",""), "title": f.get("title",""), "year": f.get("year", None)}

    return {
        "question": q,
        "answer": a,
        "citations": [cite],
        "meta": {"type": t, "value_min": lo, "value_max": hi, "unit": unit, "signal": bool(f.get("signal", False))}
    }
